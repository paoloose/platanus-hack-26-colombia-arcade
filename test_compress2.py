...
# Method A: One big string separated by '|'
combined_str = "|||".join(songs_list)
compressed_combined = zlib.compress(combined_str.encode(), 9)
b64_combined = base64.urlsafe_b64encode(compressed_combined).decode()

print(f"Method A Combined compressed size: {len(b64_combined)} chars")

indiv_total = 0
for payload in songs_list:
